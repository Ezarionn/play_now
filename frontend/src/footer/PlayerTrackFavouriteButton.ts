...
import { TrackInfo, Track } from '../main/Track/Track';
import { tracksElements } from '../index'

export class PlayerTrackFavouriteButton extends BasicFunctionality implements Creatable, Interactive {
  ...
  private isTrackFavorite(): string {
  ...
  handleClick(): void {
    ...
    this.updateElement();
    const trackFromList = this.findTrack(this.fields.title, this.fields.artist)
    if (trackFromList) {
      trackFromList.getFavouriteButton().updateElement()
      trackFromList.handleUserAction(UserAction.UPDATE, this.fields.id, 'isFavorite', this.fields.isFavorite)
    }
  }

  findTrack(title: string, artist: string): Track | undefined {
    const track = tracksElements.find(
      track => track.getTrackTitle() === title && track.getTrackArtist() === artist
    );
    return track
  }
